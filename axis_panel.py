...
class AxisPanel(ttk.LabelFrame):
    ...
    def _move(self, axis: str, delta: float) -> None:
        ...
        self._executor.submit(
            ...
            lambda error: self.after(0, self._finish_move, error),
        )

    def _finish_move(self, error: BaseException | None) -> None:
        self._set_buttons("normal")
        self._status.configure(text=str(error) if error else self._status_text())
